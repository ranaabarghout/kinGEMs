#!/bin/bash
# ---------------------------------------------------------------------
# SLURM script for Post-tuning kinGEMs validation (enzyme constraints KEPT)
# This option: Keeps enzyme constraints for knocked-out genes
# Effect: Gene knockout is total loss (no enzyme available)
# Useful for: Understanding true knockout effects
# Time: ~3-4 days
# ---------------------------------------------------------------------
#SBATCH --job-name=posttuning_enz_kept
#SBATCH --account=def-mahadeva
#SBATCH --cpus-per-task=8
#SBATCH --time=7-00:00:00
#SBATCH --mem=120G
#SBATCH --output=/project/def-mahadeva/ranaab/kinGEMs_v2/logs/posttuning_kept_%j.out
#SBATCH --error=/project/def-mahadeva/ranaab/kinGEMs_v2/logs/posttuning_kept_%j.err
...
echo "Enzyme Allocation Mode: KEPT (no reallocation)"
...
# Run post-tuning validation with enzyme constraints KEPT
echo "Running Post-tuning kinGEMs validation (enzyme kept)..."
echo "  → This simulates IMMEDIATE KNOCKOUT EFFECT"
echo "  → Knocked-out gene's enzyme remains in constraint (total loss)"
