#!/bin/bash
# ---------------------------------------------------------------------
# SLURM script for Post-tuning kinGEMs validation (enzyme constraints REMOVED)
# This option: Removes enzyme constraints for knocked-out genes
# Effect: Simulates proteome reallocation (growth may improve)
# Useful for: Understanding optimal enzyme allocation
# Time: ~3-5 days
# ---------------------------------------------------------------------
#SBATCH --job-name=posttuning_enz_removed
#SBATCH --account=def-mahadeva
#SBATCH --cpus-per-task=8
#SBATCH --time=7-00:00:00
#SBATCH --mem=120G
#SBATCH --output=/project/def-mahadeva/ranaab/kinGEMs_v2/logs/posttuning_removed_%j.out
#SBATCH --error=/project/def-mahadeva/ranaab/kinGEMs_v2/logs/posttuning_removed_%j.err
...
echo "Enzyme Allocation Mode: REMOVED (proteome reallocation)"
...
# Run post-tuning validation with enzyme constraints REMOVED
echo "Running Post-tuning kinGEMs validation (enzyme removed)..."
echo "  → This simulates PROTEOME REALLOCATION"
echo "  → Knocked-out gene's enzyme pool is freed for other reactions"
echo "  → May show growth improvements due to enzyme cost reduction"
python scripts/run_validation_parallel.py \
    --mode posttuning \
    --config configs/validation_iML1515.json \
    --output results/validation_parallel \
    --remove-knockout-enzyme
